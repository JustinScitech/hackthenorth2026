...
Vercel Hobby plus the free tiers of Tiger Data and MongoDB Atlas runs the whole demo at no cost, and it does not need `npm run worker`. When the app is deployed (`src/lib/env.ts` sees `VERCEL`), every route that enqueues a job also drains the queue after its response is sent, using Next's `after()`, so a new case is normally analysed within the same invocation. `GET`/`POST /api/jobs/run` drains on demand for retries and the 24-hour broker follow-ups; it requires `Authorization: Bearer <CRON_SECRET>` and answers `202` immediately (add `?wait=1` to drain first and get `{ processed, drained }`). `vercel.json` registers a daily Vercel Cron on that route, which is the most Hobby allows, so point a free pinger such as cron-job.org at it every minute or two with the same bearer header if you want faster retries. `INLINE_JOBS=true|false` overrides the default in either direction; locally it is off and the worker remains the processor. Both can run at once because jobs are claimed with `SKIP LOCKED` and a lease.

The app resolves databases from where it runs: locally it uses `DATABASE_URL` and `MONGODB_URI`, and on Vercel it uses `TIGERDATA_DATABASE_URL` and `MONGODB_ATLAS_URI`, refusing localhost. `./scripts/vercel-env.sh` pushes those and `CRON_SECRET` to a linked Vercel project. `.vercelignore` keeps `.env` out of uploads; never rely on `.gitignore` for that. Vercel Hobby is limited to personal, non-commercial use and 300-second invocations; a job that outlives the function keeps its lease until another drain reclaims it. The single-VM alternative in [the deployment checklist](docs/deployment.md) still runs the worker as its own container.

Atlas also has to accept connections from Vercel. The cluster's **Network Access** list is checked before authentication, and an address missing from it is refused at the TLS handshake, so a URI that works from a laptop can still fail from Vercel: `POST /api/cases` answers 503 and the runtime log shows `MongoServerSelectionError: ... tlsv1 alert internal error` with `ReplicaSetNoPrimary`. Vercel functions egress from changing AWS addresses, so either allow `0.0.0.0/0` (the database user's credentials still apply) or use a plan with static outbound IPs and list those. The list belongs to the cluster, not the connection string, so `./scripts/vercel-env.sh` cannot set it.
