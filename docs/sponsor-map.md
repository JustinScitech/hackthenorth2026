...
| Rox | Agent handling messy operational data and meaningful actions | Parser + Gemini + OpenAI resolved by agreement with confidence and visible conflicts (`src/agent/resolution.ts`); missing-field follow-up on the carrier appetite; durable job resume; 31 extraction and 15 resolution evals | Real messy Federato cases, measured resolutions and actions |
...
| Browserbase | Read an explicitly supplied public property/business source, or find one when none was supplied | Coded, optional key; the page becomes cited signals (year built, construction, size, sprinklers, flood zone) and findings that can contradict the broker (`src/agent/enrichment.ts`); without a URL a browser search is ranked into candidates the underwriter confirms (`src/agent/source-discovery.ts`); a Gemini read of the page is quote-checked and merged over the parser (`src/agent/evidence-model.ts`); 37 evals | Live browser session with cited source |
| Sentry | Observe worker failures, agent traces, and eval/job metrics without transmitting submission text | Live: web, edge, browser, and worker SDKs; job and activity spans; underwriting metrics; overview panel reads the Sentry API server-side; structured logs and gen_ai spans per model call; scrubbing pinned by 5 telemetry evals | Process a case and run `npm run eval:agent` with the DSN set, then show the trace, metrics explorer, and the /overview panel |
...
| Intact | Tenant and car quoting via a conversational assistant | `/quote`, `POST /api/quote`, and the workspace `/quotes` list; parser floor plus optional model gap-filling; 39 evals; see [docs/quoting.md](quoting.md) | Demo the journey on a phone-width screen |
