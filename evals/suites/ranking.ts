...
export function buildQueue(): { rows: Record<string, unknown>[]; bucketOf: Map<string, Bucket> } {
  ...
  for (let id = 1; id <= 60; id++) {
    ...
    if (bucket === "acceptance") Object.assign(row, [{ business_type: "renewal" }, { primary_risk_state: "GA" }, { tiv: 20_000_000 }, {}][variant]);
...
class FakeClient implements DataClient {
  ...
  async query(query: Query) {
    ...
    return { rows: ordered.slice(query.pagination.offset, query.pagination.offset + query.pagination.limit), total: ordered.length };
  }
}
