...
const flatCases: Case[] = [
  ...
  { name: "renewal is the submission-type target", row: { ...base, business_type: "renewal" }, expected: { recommendation: "Review for acceptance", score: 96, criteria: { "Submission type": "target" } }, note: "96 is the highest reachable score: line of business and construction mix have no target tier in the supplied table." },
...
const nestedBase = { id: 10, premium: 90_000, currency: "USD", business_type: "renewal", line_of_business: "property", account_name: "Nested Holdings", effective_date: "2026-03-01", expiration_date: "2027-03-01" };

const nestedCases: Case[] = [
  { name: "nested: building TIV sums and the oldest building governs age", row: { ...nestedBase, exposure_units: [{ location: location(1, "OH", [building(1), building(2, { year_built: 1995 })]) }], claims: [] }, expected: { recommendation: "Review for acceptance", criteria: { "Total insured value": "target", "Building age": "acceptable", "Primary risk state": "target", "Five-year loss value": "target", "Construction mix": "acceptable" } } },
  { name: "nested: the same location referenced twice is counted once", row: { ...nestedBase, exposure_units: [{ location: location(1, "OH", [building(1)]) }, { location: location(1, "OH", [building(1)]) }], claims: [] }, expected: { recommendation: "Review for acceptance", criteria: { "Total insured value": "acceptable" } }, note: "Double-counting a shared location would push $30M into the $50M target band." },
  ...
  { name: "nested: claims older than five years are excluded", row: { ...nestedBase, exposure_units: [{ location: location(1, "OH", [building(1)]) }], claims: [claim(1, "2019-05-01", 500_000), claim(2, "2025-06-01", 10_000)] }, expected: { recommendation: "Review for acceptance", criteria: { "Five-year loss value": "target" } } },
