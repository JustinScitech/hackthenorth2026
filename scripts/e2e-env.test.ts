...
test("E2E cannot target hosted storage or Temporal Cloud", () => {
  ...
  try {
    ...
    assert.equal(env.TEMPORAL_ADDRESS, "127.0.0.1:7233");
    assert.equal(env.TEMPORAL_NAMESPACE, "default");
    assert.equal(env.TEMPORAL_API_KEY, "");
  } finally {
