    ...
    <main className="shell">
      ...
      <div className="stat-row">
        ...
        <Stat label="System status" hint="Whether the case API is reachable from this browser">
          <span className={`system-status${error ? " degraded" : ""}`}>{error ? <WarningCircle size={16} /> : <SealCheck size={16} weight={loading ? "regular" : "fill"} />}{error ? "API unreachable" : loading ? "Checking" : "All systems operational"}</span>
        </Stat>
      </div>
