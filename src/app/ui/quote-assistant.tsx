...
import { Car, CircleAlert, House, MessageSquareText, Send, ShieldCheck, UserRound } from "lucide-react";
  ...
  return <div className="quote-shell">
    <ol className="quote-thread" aria-label="Conversation">
      {messages.map((message) => <li key={message.id} className={`conversation-message ${message.role === "you" ? "request-message" : "agent-message"}`}>
        <div className={`message-avatar ${message.role === "you" ? "requester-avatar" : "agent-avatar"}`}>{message.role === "you" ? <UserRound size={16} aria-hidden="true" /> : <ShieldCheck size={18} aria-hidden="true" />}</div>
    ...
    <div aria-live="polite" className="quote-live">
      {busy && <p className="progress-line" role="status"><MessageSquareText size={16} aria-hidden="true" />Working out your estimate…</p>}
      ...
      {result && <section className="quote-result" aria-label="Your estimate">
        ...
        {result.status === "refer" && <div className="notice"><CircleAlert size={17} aria-hidden="true" />{result.recommendation}</div>}
    ...
    <form className="quote-composer" onSubmit={send}>
      ...
      <button className="primary-button" type="submit" disabled={busy || !text.trim()}><Send size={15} aria-hidden="true" />Send</button>
    </form>
  </div>;
}
