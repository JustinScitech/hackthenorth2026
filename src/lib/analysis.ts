...
export function parseBrokerNotes(text: string): Extracted {
  ...
  for (const match of text.matchAll(/(?:losses|claims)(?:\s+in\s+(?:the\s+)?(?:last|past)\s+\d+\s+years?)?\s*[:#-]?\s*(\d+)\b/gi)) {
    lossCandidates.push({ index: match.index, value: Number(match[1]) });
  }
  for (const match of text.matchAll(/\b(\d+)\s+(?:recent\s+)?(?:losses|claims)\b/gi)) {
    lossCandidates.push({ index: match.index, value: Number(match[1]) });
  }
