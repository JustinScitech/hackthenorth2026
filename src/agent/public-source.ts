...
export async function browsePublicSource(value: string, capture: (value: string) => Promise<CapturedPage> = capturePublicPage): Promise<PublicEvidence> {
  const { url, title, text } = await capture(value);
...
export async function capturePublicPage(value: string): Promise<CapturedPage> {
